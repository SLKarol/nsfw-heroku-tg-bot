...
_todo_ разобраться, почему не показывает в UI некоторое ~~дерьмо~~ видео

_todo_ переехать от модульного css

_todo_ Научить бот сводить аудио и видеодорожку.

_todo_ задействовать /api/isFriday

_todo_ UI научить реагировать на истечение времени действия токена

_todo_ Сделать выбор каналов в телеграм через кнопки
